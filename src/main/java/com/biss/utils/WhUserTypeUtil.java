...
public class WhUserTypeUtil {
	public void generatePie(String path,List<Object[]> data) {
		...
		JFreeChart chart=ChartFactory.createPieChart3D("Whuser Id Type", dataset, true, true, false);
	...
	public void generateBar(String path,List<Object[]> data) {
		...
		//2. Create JFreeChart using ChartFactory
		JFreeChart chart=ChartFactory.createBarChart("Whuser Id Type", "Modes", "COUNT", dataset);
		//3. Save as Image using ChartUtil
		try { ChartUtils.saveChartAsJPEG(new File(path+"/resources/images/whuserBar.jpg"), chart, 400, 400); } catch (IOException e) {
			e.printStackTrace();
		}
	}
}
