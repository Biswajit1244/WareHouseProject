...
public class OrderMethodUtil {
	public void generatePie(String path,List<Object[]> data) {
		...
		JFreeChart chart=ChartFactory.createPieChart3D("OrderMethod Order Type", dataset, true, true, false);
		//3. save as Image using ChartUtils
		try {
			ChartUtils.saveChartAsJPEG(new File(path+"/resources/images/orderMethodPie.jpg"), chart, 400, 400);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public void generateBar(String path,List<Object[]> data) {
		...
		//2. Create JFreeChart using ChartFactory
		JFreeChart chart=ChartFactory.createBarChart("OrderMethod Order Type", "Modes", "COUNT", dataset);
		//3. Save as Image using ChartUtil
		try { ChartUtils.saveChartAsJPEG(new File(path+"/resources/images/orderMethodBar.jpg"), chart, 400, 400); } catch (IOException e) {
			e.printStackTrace();
		}
	}
}
