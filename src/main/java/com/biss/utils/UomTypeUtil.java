...
public class UomTypeUtil {
	...
	public void generateBar(String path,List<Object[]> data) {
		...
		//2. Create JFreeChart using ChartFactory
		JFreeChart chart=ChartFactory.createBarChart("Uom Type", "Modes", "COUNT", dataset);
		//3. Save as Image using ChartUtil
		try { ChartUtils.saveChartAsJPEG(new File(path+"/resources/images/uomTypeBar.jpg"), chart, 400, 400); } catch (IOException e) {
			e.printStackTrace();
		}
	}
}
